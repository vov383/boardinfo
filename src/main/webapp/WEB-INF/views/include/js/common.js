...
function getFileInfo(fullName){
	...
	if(checkImageType(fullName)) {//이미지 파일인 경우
		imgsrc="/board-info/upload/displayFile?fileName="+fullName;
		fileLink=fullName.substr(14); //14인덱스~끝
		var front = fullName.substr(0, 12); //0~11
		var end = fullName.substr(14);
		
		getLink="/board-info/upload/displayFile?fileName=" + front + end;
	}else{//이미지가 아닌 경우
		fileLink=fullName.substr(12);
		getLink="/board-info/upload/displayFile?fileName=" + fullName;
	}
