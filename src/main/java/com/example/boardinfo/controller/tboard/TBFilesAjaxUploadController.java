...
public class TBFilesAjaxUploadController {

	//로깅
	private static Logger logger = 
			LoggerFactory.getLogger(TBFilesAjaxUploadController.class);
	...
	// 그 역은 @RequestBody
	@RequestMapping(value = "attachAjax",
	...
	public ResponseEntity<String> uploadAjax(
		...
		ResponseEntity<String> uploadedImg = new ResponseEntity<String>(UploadFileUtils.uploadFile(
				uploadPath, file.getOriginalFilename(),
				file.getBytes()), HttpStatus.OK);

		TBAttachDTO f_dto = new TBAttachDTO();
		//파일 업로드 전에 tb_files 테이블에 담을 데이터 추출
		String fullName = UploadFileUtils.uploadFile(
				uploadPath, file.getOriginalFilename(), file.getBytes());
		logger.info("fullName : "+ fullName);
		byte[] fileData = file.getBytes();
		int idx = fullName.lastIndexOf(".");
		String formatName = fullName.substring(idx+1);
		logger.info("formatName : "+ formatName);

		String userid = (String)session.getAttribute("userid");
		userid = "testuser";
		f_dto.setFullName(fullName);
		f_dto.setFileData(fileData);
		f_dto.setFormatName(formatName);
		f_dto.setCreate_user(userid);
