...
@Controller
@RequestMapping("tbUpload/*")
public class TBUploadController {

    //로깅
    private static Logger logger =
            LoggerFactory.getLogger(TBUploadController.class);
    ...
    // 그 역은 @RequestBody
    @PostMapping(value = "/uploadAjax", produces = "text/plain;charset=utf-8") //한글이 깨지지 않도록 보완처리

    // ResponseEntity : 업로드한 파일 정보와 Http 상태 코드를 함께 리턴
    public ResponseEntity<String> uploadAjax(@RequestParam MultipartFile file)
            throws Exception {
    ...
    @ResponseBody
    @GetMapping("/displayFile/file_reg_num.{file_reg_num}")
    ...
    @PutMapping(value = "deleteFile/file_reg_num.{file_reg_num}")
